import * as base from './_base'
import { COLLECTIONS, STORAGE_PATHS } from '../utils/constants'
import { uploadMany } from './storage.service'

const C = COLLECTIONS.MEMORIES

export const list = (opts) => base.list(C, opts)
export const subscribe = (cb, opts) => base.subscribe(C, cb, opts)
export const get = (id) => base.getOne(C, id)
export const remove = (id) => base.remove(C, id)

export const create = async (uid, data, files = {}) => {
  const stamp = Date.now()
  const photos = await uploadMany(`${STORAGE_PATHS.MEMORIES}/${uid}/${stamp}/photos`, files.photos || [])
  const videos = await uploadMany(`${STORAGE_PATHS.MEMORIES}/${uid}/${stamp}/videos`, files.videos || [])
  return base.create(C, {
    title: data.title || '',
    description: data.description || '',
    date: data.date || new Date().toISOString().slice(0, 10),
    location: data.location || '',
    photos: [...(data.existingPhotos || []), ...photos],
    videos: [...(data.existingVideos || []), ...videos],
    tags: data.tags || [],
    createdBy: uid,
  })
}

export const update = async (uid, id, data, files = {}) => {
  const photos = await uploadMany(`${STORAGE_PATHS.MEMORIES}/${uid}/${id}/photos`, files.photos || [])
  const videos = await uploadMany(`${STORAGE_PATHS.MEMORIES}/${uid}/${id}/videos`, files.videos || [])
  return base.update(C, id, {
    title: data.title,
    description: data.description,
    date: data.date,
    location: data.location || '',
    photos: [...(data.existingPhotos || []), ...photos],
    videos: [...(data.existingVideos || []), ...videos],
    tags: data.tags || [],
  })
}
